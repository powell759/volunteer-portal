...
from models import Initiative, VolunteerEvent, VolunteerRole, DonationEmail, Notification, Account, PersonalIdentifier, VerificationToken
...
try:
    ...
    if ENV == 'development':
        logging.warning('Unable to connect to secret store, falling back to config.yaml')
    else:
    ...
    for auth_key in config['auth']:
        if auth_key == 'import_auth_credentials_from_secret_store':
            ...
            for credential_key in config['auth'][auth_key]:
                config['auth'][auth_key][credential_key] = get_secret_for_key(config['auth'][auth_key][credential_key])
